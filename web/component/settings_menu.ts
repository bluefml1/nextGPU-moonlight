...
export type Settings = {
    ...
    /**
     * Host-side folder for uploads (drag-drop / HTTP): a single relative name like `Desktop`, not a full path.
     * Default `Desktop`. Empty still means Desktop.
     */
    hostUploadRelativeDir: string
}
...
function getVideoPresetOptions(): { value: string; name: string }[] {
    const lastTierMbps = (typeof window !== "undefined" && (window as any).mlLastSpeedtestTierMbps) ?? 40
...
export class StreamSettingsComponent implements Component {
    ...
    public divElement: HTMLDivElement = document.createElement("div")

    // Section wrappers for layout (sidebar + content panels)
    private sidebarSection: HTMLDivElement
    private streamSection: HTMLDivElement
    private audioSection: HTMLDivElement
    private mouseSection: HTMLDivElement
    ...
    private hostUploadHeader: HTMLHeadingElement = document.createElement("h2")
    private hostUploadRelativeDir: InputComponent
    ...
    constructor(settings?: Settings) {
        ...
        this.videoSize = new SelectComponent("videoPreset",
        ...
        this.hostUploadRelativeDir = new InputComponent(
            "hostUploadRelativeDir",
            "text",
            "Host upload folder",
            {
                defaultValue: "",
                value:
                    settings?.hostUploadRelativeDir ?? defaultSettings_.hostUploadRelativeDir ?? "",
                placeholer: "Desktop",
            }
        )

...
export type SelectInit = {
    ...
    displayName?: string,
    /** Force using the polyfill dropdown even if native <select> is supported. */
    forcePolyfill?: boolean
    /** Adds a CSS class to the polyfill list container. */
    listClass?: string
    /** Render an embedded label in the closed display (two-line style). */
    embeddedLabel?: string
    /** Optional label shown when closed (UI hint for polyfill). */
    closedDisplayLabel?: string | null
    /** Prefer opening the polyfill list upwards (CSS hook). */
    forceTop?: boolean
}
...
type SelectStrategy =
    ...
    { name: "polyfill", opened: boolean, wrapper: HTMLDivElement, display: HTMLParagraphElement, list: HTMLDivElement, value: string | null, disabled: Set<string>, closedDisplayLabel: string | null, forceTop: boolean }
...
export class SelectComponent extends ElementWithLabel {
    ...
    constructor(internalName: string, options: Array<{ value: string, name: string }>, init?: SelectInit) {
        ...
        // Create base
        if (init?.forcePolyfill || useSelectElementPolyfill() || !isElementSupported("select")) {
            ...
            this.strategy = {
                ...
                disabled: new Set(),
                closedDisplayLabel: init?.closedDisplayLabel || null,
                forceTop: !!init?.forceTop,
            }

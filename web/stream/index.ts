...
export class Stream implements Component {
    ...
    private lastAppliedBitrateKbps: number | null = null
    ...
    /** Relative host folder for drag/drop uploads (e.g. `Desktop`); empty => Desktop. Not a full path. */
    private hostUploadRelativeDir: string
    ...
    private async onDrop(event: DragEvent) {
        ...
        await this.uploadFileToHost(files[0])
    }
    ...
    private emitFileTransferProgress(
        ...
        } = { type: "fileTransferProgress", percent, source }
        if (loaded !== undefined) detail.loaded = loaded
        if (total !== undefined) detail.total = total
        const event: InfoEvent = new CustomEvent("stream-info", { detail })
        this.eventTarget.dispatchEvent(event)
    }

    private emitFileTransferProgressEnd(source: "file" | "clipboard" = "file") {
     ...
     */
    private postHostPayloadWithProgress(
        path: "/host/upload" | "/host/clipboard",
        body: Blob,
        source: "file" | "clipboard",
        options: { logLabel: string; extraHeaders?: Record<string, string> }
    ): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (options.extraHeaders) {
                ...
                for (const k of Object.keys(extra) as Array<keyof typeof extra>) {
            ...
            xhr.onload = () => {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    this.debugLog(
                        `${source === "clipboard" ? "[Clipboard]" : "[Upload]"} Completed: ${options.logLabel}`
                    )
                    resolve()
                } else {
                    this.emitFileTransferProgressEnd(source)
                    this.debugLog(
                        `${source === "clipboard" ? "[Clipboard]" : "[Upload]"} Failed (${xhr.status}): ${options.logLabel}`
                    )
            ...
            xhr.onerror = () => {
                this.emitFileTransferProgressEnd(source)
                this.debugLog(
                    `${source === "clipboard" ? "[Clipboard]" : "[Upload]"} Network error: ${options.logLabel}`
                )
    ...
    uploadFileToHost(file: File): Promise<void> {
     ...
     */
    syncClipboardToHost(text: string): Promise<void> {
    ...
    private shouldEmitLog(message: string, additional?: LogMessageInfo): boolean {
        ...
        return (
            ...
            message.includes(MOONLIGHT_BITRATE_LOG_PREFIX)
        )
    }
    ...
    private async onMessage(message: StreamServerMessage) {
        if ("DebugLog" in message) {
            ...
            if (debugLog.message.includes(MOONLIGHT_BITRATE_LOG_PREFIX)) {
                console.info(debugLog.message)
            }
    ...
    private resetKeyboardState(reason: string) {
    ...
    /**
     * While streaming, send realtime bitrate to the host (debounced for slider-like sources).
     */
    requestVideoBitrateKbps(kbps: number, source: VideoBitrateChangeSource, opts?: { immediate?: boolean }): void {
        const immediate = opts?.immediate === true
        const rounded = Math.round(kbps)
        if (!Number.isFinite(rounded) || rounded < 500) {
            return
        }

        if (!this.streamingVideoActive) {
            console.info(
                `${MOONLIGHT_BITRATE_LOG_PREFIX} request: not streaming; skip send from=${this.lastAppliedBitrateKbps ?? "n/a"} to=${rounded} source=${source}`
            )
            return
        }

        const fromKbps = this.lastAppliedBitrateKbps ?? this.settings.bitrate
        const doSend = () => {
            this.lastAppliedBitrateKbps = rounded
            console.info(`${MOONLIGHT_BITRATE_LOG_PREFIX} request: from=${fromKbps} to=${rounded} source=${source}`)
            this.sendWsMessage({ SetVideoBitrate: { bitrate_kbps: rounded } })
        }
    ...
    noteLocalBitrateKbps(kbps: number): void {
        const r = Math.round(kbps)
        if (Number.isFinite(r) && r >= 500) {
            this.lastAppliedBitrateKbps = r
        }
    }

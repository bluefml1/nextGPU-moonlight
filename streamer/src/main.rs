...
impl StreamConnection {
    ...
    async fn set_transport(
        ...
        if let Err(err) = self.release_all_pressed_keys().await {
            warn!("Failed to release keys before switching transport: {err:?}");
        }
        let this = self.clone();
    ...
    async fn on_packet(&self, packet: InboundPacket) {
        ...
        let err = match packet {
            ...
            InboundPacket::Key {
                ...
                match action {
                    moonlight_common::stream::control::KeyAction::Down => {
                        pressed_keys.insert(key);
                    }
                    moonlight_common::stream::control::KeyAction::Up => {
    ...
    async fn release_all_pressed_keys(&self) -> Result<(), MoonlightError> {
        let keys_to_release: Vec<u16> = {
            let mut pressed_keys = self.pressed_keys.lock().await;
            pressed_keys.drain().collect()
        };
        ...
        for key in keys_to_release {
            stream.send_keyboard_event_non_standard(
                key as i16,
                moonlight_common::stream::control::KeyAction::Up,
                moonlight_common::stream::control::KeyModifiers::empty(),
                moonlight_common::stream::control::KeyFlags::empty(),
            )?;
        }
    ...
    async fn start_stream(self: &Arc<Self>, settings: StreamSettings) -> Result<(), anyhow::Error> {
        ...
        if let Err(err) = self.release_all_pressed_keys().await {
            warn!("Failed to release keys before starting new stream: {err:?}");
        }
    ...
    async fn stop(&self) {
        ...
        if let Err(err) = self.release_all_pressed_keys().await {
            warn!("Failed to release pressed keys while stopping: {err:?}");
        }
